...
from grad_cam_fpn import GradCAM, GradCamPlusPlus
...
def get_last_conv_name(net):
    ...
    for name, m in net.named_modules():
        ...
        if name == 'backbone.bottom_up.res5.2.conv3':
            # if name == 'roi_heads.pooler':
            layer_name = name
    return layer_name
...
def main(args):
    ...
    image = torch.as_tensor(original_image.astype("float32").transpose(2, 0, 1)).requires_grad_(True)

    inputs = {"image": image, "height": height, "width": width}

    # Grad-CAM
    layer_name = get_last_conv_name(model)
    print('get_last_conv_name ==>', layer_name)
    grad_cam = GradCAM(model, layer_name, original_img=original_image)
    mask, box, class_id = grad_cam(inputs, index=64)  # cam mask why 这里的index只能是0？
